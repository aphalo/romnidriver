...
CRAN. 'rJava' requires that the Java JDK, the development kit, is installed.
Installed only Java JRE runtime is not enough in this case, although it is for
many other programs written in Java.
...
Depending on the operating system, manually setting the environment variable `"JAVA_HOME"` may be necessary. We can check if this variable is set either at the operating system command prompt or from R, as shown here. Windows uses a "registery" to keep track of the location of programs so even if this variable is not set, 'rJava' will find the Java JDK under Windows. 
