...
This package provides a wrapper on the OmniDriver from Ocean Insight. The 
runtime can be downloaded and installed for free. Please, use the Java
documentation provided by Ocean Insight. The names of all functions are
...
Optionally one force the printing of a diagnosys message in the abscence even
for a sucesful loading of the package. We do it here to illustrate the location
of the driver and Java.
...
The next step is to open connections to all Ocean Insight spectrometers
...
but this can also be disabled. (According to Ocean Insight' literature the
